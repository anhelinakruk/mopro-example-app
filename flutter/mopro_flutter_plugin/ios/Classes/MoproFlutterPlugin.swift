...
public class MoproFlutterPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "generateNoirProof":
      ...
      do {
        let vk = try getNoirVerificationKey(circuitPath: circuitPath, srsPath: srsPath, onChain: false, lowMemoryMode: false)
        let proofResult = try generateNoirProof(
          circuitPath: circuitPath, srsPath: srsPath, inputs: inputs, onChain: false, vk: vk, lowMemoryMode: false)
        result(proofResult)
      } catch {
        result(
          FlutterError(
    ...
    case "verifyNoirProof":
      ...
      do {
        let vk = try getNoirVerificationKey(circuitPath: circuitPath, srsPath: nil, onChain: false, lowMemoryMode: false)
        let valid = try verifyNoirProof(circuitPath: circuitPath, proof: proof.data, onChain: false, vk: vk, lowMemoryMode: false)
        result(valid)
      } catch {
        result(
          FlutterError(
